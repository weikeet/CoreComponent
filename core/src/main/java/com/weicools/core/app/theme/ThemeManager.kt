...
package com.weicools.core.app.theme
...
import com.weicools.core.app.BaseThemeActivity
...
class ThemeManager {
  ...
  private val liveTheme: MutableLiveData<AppTheme> = MutableLiveData()
  private lateinit var activity: BaseThemeActivity

  fun init(activity: BaseThemeActivity, defaultTheme: AppTheme) {
  ...
  fun setActivity(activity: BaseThemeActivity) {
    this.activity = activity
  }

  fun getCurrentTheme(): AppTheme? {
    return liveTheme.value
  }
  ...
  fun changeTheme(newTheme: AppTheme, sourceCoordinate: Coordinate, duration: Long = 600, isRevers: Boolean = false) {
    ...
    // set LiveData
    liveTheme.value = newTheme
  }

  //region StatusBar NavigationBar
  ...
  fun syncNavigationBarButtonsColorWithBackground(activity: Activity, navigationBarBackgroundColor: Int) {
  ...
  private fun getRelativeLeft(myView: View): Int {
    return if ((myView.parent as View).id == R.id.fake_theme_root_view) {
      myView.left
    } else {
